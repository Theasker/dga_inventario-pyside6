...
class InventarioModel:
    ...
    def crear_estructura_si_no_existe(self):
        with self.conectar() as conn:
            ...
            cursor.execute(
                ...
                ubicacion_id INTEGER NOT NULL, usuario_id INTEGER,
    ...
    def obtener_inventario_completo(self, texto="", seccion_id=0):
        query = """
        SELECT d.id, u.nombre, usr.nombre, usr.correo, td.nombre, d.marca, d.modelo, d.fecha_registro, d.observaciones
        FROM dispositivos d
        ...
        params = []
        if texto:
            query += " AND (usr.nombre LIKE ? OR d.marca LIKE ? OR d.modelo LIKE ? OR u.nombre LIKE ?)"
            term = f"%{texto}%"
    ...
    def actualizar_dispositivo_completo(self, d_id, datos):
        try:
            with self.conectar() as conn:
                ...
                cursor.execute(
                    "UPDATE dispositivos SET tipo_dispositivo_id=?, marca=?, modelo=?, observaciones=? WHERE id=?",
                    (datos["tipo_id"], datos["marca"], datos["modelo"], datos["observaciones"], d_id)
                )
                # Actualizar tabla asignaciones
                cursor.execute(
                    "UPDATE asignaciones SET ubicacion_id=?, usuario_id=? WHERE dispositivo_id=?",
    ...
    def importar_desde_csv(self, ruta_archivo, progreso_callback=None):
        import csv
        try:
            # Usamos utf-8-sig para limpiar el BOM de Excel
            with open(ruta_archivo, 'r', encoding='utf-8-sig') as f:
                # Filtrar líneas que realmente tengan datos
                lineas_reales = [l for l in f.readlines() if l.strip() and not l.strip().startswith(';')]
                total_filas = len(lineas_reales) - 1
                f.seek(0)
                
                # Detectamos si usa coma o punto y coma
                cabecera = f.readline()
                sep = ';' if ';' in cabecera else ','
                f.seek(0)
                ...
                with self.conectar() as conn:
                    ...
                    for i, fila_raw in enumerate(lector):
                        # LIMPIEZA DE CLAVES: Quita espacios en blanco de los nombres de las columnas
                        fila = {k.strip(): v for k, v in fila_raw.items() if k is not None}
                        
                        # Si la fila no tiene datos clave, saltar
                        if not fila.get('Sección') or not fila['Sección'].strip():
                            continue
                        ...
                        # 3. Usuario y Correo
                        u_nom = fila.get('Apellidos y nombre', 'Desconocido').strip()
                        u_mail = fila.get('Correo', '').strip()
                        cursor.execute("INSERT OR IGNORE INTO usuarios (nombre, correo) VALUES (?, ?)", (u_nom, u_mail))
                        cursor.execute("SELECT id FROM usuarios WHERE nombre = ?", (u_nom,))
                        ...
                        # 6. Asignación
                        fecha = fila.get('Fecha', '2026-01-01')
                        cursor.execute('''INSERT INTO asignaciones (dispositivo_id, ubicacion_id, usuario_id, fecha)
                                          VALUES (?, ?, ?, ?)''', (d_id, l_id, u_id, fecha))

                        if progreso_callback and total_filas > 0:
...
class InventarioVista(QMainWindow):
    ...
    def _create_layout(self):
        ly_sup = QHBoxLayout()
        ly_sup.addWidget(QLabel("Sección:"))
        ly_sup.addWidget(self.cmb_seccion)
        ly_sup.addWidget(self.btn_add_seccion)
        ly_sup.addWidget(self.btn_gear_seccion)
        ly_sup.addWidget(self.btn_add_lugar)
        ly_sup.addWidget(self.btn_gear_lugar)
        ly_sup.addSpacing(20)
        ly_sup.addWidget(QLabel("🔍"))
        ly_sup.addWidget(self.txt_busqueda)
        ly_sup.addWidget(self.btn_add_user)
        ly_sup.addWidget(self.btn_gear_user)
        ly_sup.addWidget(self.btn_add_dispositivo)
        ly_sup.addWidget(self.btn_importar)
        self._layout_principal.addLayout(ly_sup)
...
class InventarioControlador:
    ...
    def guardar_cambios_edicion(self, fila, d_id):
        try:
            id_ubicacion = self.vista.tabla_inventario.cellWidget(fila, 1).currentData()
            marca = self.vista.tabla_inventario.item(fila, 5).text().strip()
            modelo = self.vista.tabla_inventario.item(fila, 6).text().strip()

            if not id_ubicacion or not marca or not modelo:
                return QMessageBox.warning(self.vista, "Error", "Lugar, Marca y Modelo son obligatorios.")
            ...
            QMessageBox.critical(self.vista, "Error", f"No se pudo actualizar: {e}")
    ...
    def importar_datos(self):
        ...
        if ruta:
            progreso = QProgressDialog("Importando 180 registros...", "Cancelar", 0, 100, self.vista)
            ...
            if exito:
                self.cargar_filtros()
                self.actualizar_tabla()
                QMessageBox.information(self.vista, "Éxito", "Importación terminada. Los datos ya están visibles.")
            else:
                QMessageBox.critical(self.vista, "Error", "No se pudo leer el archivo. Revisa que los encabezados sean correctos.")
